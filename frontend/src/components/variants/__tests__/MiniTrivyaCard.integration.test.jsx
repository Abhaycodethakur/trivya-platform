...
// Mock parent component that uses MiniTrivyaCard
const TestPage = () => {
  ...
  return (
    <div>
      ...
      <div className="card-container" style={{ display: 'flex', gap: '20px' }}>
        <MiniTrivyaCard 
          onSeeDetails={() => handleSeeDetails('mini-trivya')}
          isSelected={selectedCard === 'mini-trivya'}
          data-testid="mini-trivya-card"
        />
      </div>
    </div>
  );
...
describe('MiniTrivyaCard Integration', () => {
  it('handles click interaction within parent component', () => {
    ...
    expect(screen.getByText('Details for mini-trivya')).toBeInTheDocument();
