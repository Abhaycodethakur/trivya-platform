...
// Mock parent component that uses TrivyaHighCard
const TestPage = () => {
  ...
  return (
    <div>
      ...
      <div className="card-container" style={{ display: 'flex', gap: '20px' }}>
        <TrivyaHighCard 
          onSeeDetails={() => handleSeeDetails('trivya-high')}
          isSelected={selectedCard === 'trivya-high'}
          data-testid="trivya-high-card"
        />
      </div>
    </div>
  );
...
describe('TrivyaHighCard Integration', () => {
  it('handles click interaction within parent component', () => {
    ...
    expect(screen.getByText('Details for trivya-high')).toBeInTheDocument();
